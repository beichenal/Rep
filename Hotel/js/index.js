$(document).ready(function () {
    ...
    //ajax请求
    {
        ...
        li = "<li class='body-remeng'>热门酒店</li>";
        li += "<li role='presentation' class='active'><a href='#hot-city" + 1 +
            "' aria-controls='hot-city1' role='tab' data-toggle='tab'>" + data.data.hot[0].cityName + "</a></li>";
        for (let i = 1; i < data.data.hot.length; i++) {
            li += "<li role='presentation'><a href='#hot-city" + i +
                "' aria-controls='hot-city" + i + "' role='tab' data-toggle='tab'>" + data.data.hot[i].cityName + "</a></li>";
        }
        $(".body-tab .body-ul").html(li);
    ...
    //ajax 开始
    {
        ...
        for (let i = 0; i < 10; i++) {
            li += "<li>" +
                "<div class='jiudian'>" +
                "<span id='jiudian-bottom'>" +
                "<span id='jiudian-name'>" + "西安开元名都大酒店" + "</span>" +
                "<span id='jiudian-style'>" + "豪华型" + "</span>" +
                "</span>" +
                "<span id='jiudian-money'>" + "¥488" + "</span>" +
                "</div>" +
                "</li>"
        }
        let div = "<div role='tabpanel' class='tab-pane active hot-city' id='" + $(".body-tab .body-ul .active a").attr("aria-controls") + "'>" + "<ul>" + li + "</ul>" + "</div>"
    ...
    //ajax开始
    {
        $(".body-tab .body-ul li").click(function (e) {
            let li = ""
            for (let i = 0; i < 10; i++) {
                li += "<li>" +
                    "<div class='jiudian'>" +
            ...
            let div = "<div role='tabpanel' class='tab-pane active hot-city' id='" + $(e.target).attr("id") + "'>" + "<ul>" + li + "</ul>" + "</div>"
    ...
    let date = new Date();
    $("#date-year").text(date.getFullYear())
    $("#date-month").text(date.getMonth() + 1)
    $("#date-year-right").text(date.getFullYear())
    $("#date-month-right").text(date.getMonth() + 1)



    function my_date(mark1, mark2, mark3) {
        let date2 = new Date($(mark2).text(), $(mark3).text(), 0);
        let day_number = date2.getDate();
        var count = 1;
        date2.setDate(1);
        var weekd = date2.getDay()
        console.log(weekd);
        var tr = ""
        for (let i = 1; i <= (Math.ceil(day_number / 7)) + 1; i++) {
            tr += "<tr>";
            for (let j = 0; j <= 6; j++) {
                if (i == 1 && j < weekd) {
                    tr += "<td></td>"
                } else {
                    if (count == date.getDate() && date2.getFullYear() == date.getFullYear() && date2.getMonth() == date.getMonth()) {
                        tr += "<td class='date-td today act' data-days=" + count + ">" + "今天" + "</td>";
                        count++;
                    } else {
                        if (count > day_number) {
                            break;
                        }
                        tr += "<td class='date-td' data-days=" + count + ">" + count + "</td>"
                        count++;
                    }
    ...
    my_date("#tab-l tbody ", "#date-year", "#date-month");
    my_date("#tab-r tbody ", "#date-year-right", "#date-month-right");





    $("#date-left-top-year").click(function () {
        $("#date-year").text(parseInt($("#date-year").text()) - 1)
        my_date("#tab-l tbody ", "#date-year", "#date-month");
    })
    $("#date-left-top-month").click(function () {
        let month = parseInt($("#date-month").text()) - 1;
        if (month == 0) {
            $("#date-year").text(parseInt($("#date-year").text()) - 1)
            month = 12;
            $("#date-month").text(month);
        } else {
            $("#date-month").text(parseInt($("#date-month").text()) - 1)
            my_date("#tab-l tbody ", "#date-year", "#date-month");
        }
    })
    ...
    $("#date-right-top-year").click(function () {
        $("#date-year-right").text(parseInt($("#date-year-right").text()) + 1)
        my_date("#tab-r tbody ", "#date-year-right", "#date-month-right");
    })
    $("#date-right-top-month").click(function () {
        let month = parseInt($("#date-month-right").text()) + 1;
        if (month == 13) {
            $("#date-year-right").text(parseInt($("#date-year-right").text()) + 1)
            month = 1;
            $("#date-month-right").text(month);

        } else {
            $("#date-month-right").text(parseInt($("#date-month-right").text()) + 1)
            my_date("#tab-r tbody ", "#date-year-right", "#date-month-right");
        }
    })
    ...
    $("#tab-l").click(function (e) {
        let date4 = new Date(date.getFullYear(), date.getMonth(), date.getDate())
        let date3 = new Date($("#date-year").text(), $("#date-month").text() - 1, $(e.target).data("days"));
        if (date3.getTime() >= date4.getTime()) {
            $("#tab-l tbody .date-td").removeClass("act");
            $(e.target).toggleClass("act")
        }
    })
    $("#tab-r").click(function (e) {
        let date4 = new Date($("#date-year").text(), $("#date-month").text() - 1, $("#tab-l tbody .act").data("days"));
        let date3 = new Date($("#date-year-right").text(), $("#date-month-right").text() - 1, $(e.target).data("days"));
        if (date3.getTime() >= date4.getTime()) {
            $("#tab-r tbody .date-td").removeClass("act");
            $(e.target).toggleClass("act")
            let str = $("#date-year").text() + "-" + $("#date-month").text() + "-" + $("#tab-l .act").data("days") + "至" + $("#date-year-right").text() + "-" + $("#date-month-right").text() + "-" + $(e.target).data("days");
            $("#date").text(str);
            $("#date-tab").toggleClass("hidden");
        }
    })
